...
# ffmpeg_time设置
ffmpeg_time(){
  read -p "请输入推流时间(直接回车将使用默认值[$time]):" ||true
  # 如果用户有输入，使用用户的输入
  if [[ -n "$REPLY" ]]; then
    time="$REPLY"
  fi
  # 判断用户输入的是否是两位数字:两位数字:两位数字
  if [[ $time =~ ^[0-9]{2}:[0-9]{2}:[0-9]{2}$ ]]; then
    echo -e "${yellow}推流时间为:$time ${font}"
  else
    echo -e "${red}你输入的时间不合法,请重新运行程序并输入! ${font}"
    exit 1
  fi
}
# ffmpeg帧率设置
...
stream_start() {
  ...
  if [ "$watermark" = "yes" ]; then
    ...
    while true; do
      ...
      nohup ffmpeg -ss "$time" -re -i "$video" -i "$image" -preset ultrafast -vcodec libx264 -r "$fps" -g 60 -b:v "$rate" -c:a aac -b:a 92k -strict -2 -f flv "${rtmp}" > output.log 2>&1 &
    done
  fi
  if [ "$watermark" = "no" ]; then
    ...
    while true; do
      ...
      nohup ffmpeg -ss "$time" -re -i "$video" -preset ultrafast -vcodec libx264 -r "$fps" -g 60 -b:v "$rate" -c:a aac -b:a 92k -strict -2 -f flv "${rtmp}" > output.log 2>&1 &
      time="00:01:40"
      echo -e "${yellow}重制起始时间为$time ${font}"
      stream_log
    done
  fi
}
...
![image-20230404005242976](http://image.hi-hufei.com/typora/image-20230404005242976.png)

#### 2.1安装前置依赖

根据提示选择`第0项`会自动检查是否安装并安装ffmpeg和screen

后续推流按照提示一步一步走就行了

![image-20230404010739036](http://image.hi-hufei.com/typora/image-20230404010739036.png)

### 3.注意

`目前上面的方式不支持后台运行，也就是ssh断开就关闭了。下面介绍如果挂载后台`
...
如果总是按照2.1的提示来操作也挺麻烦的，故也支持直接在文本中配置后续只需要一路确认下去就OK了。如图：

``` shell
vim ./live.sh
```

编辑shell这里面有常用的配置最后编辑好保存即可

![image-20230404010226835](http://image.hi-hufei.com/typora/image-20230404010226835.png)
